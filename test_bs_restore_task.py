...
class TestRestore:
    def __init__(self):
        ...
        try:
            bh_mgr = bhcli_helper.BHManager("./fake_bhcli_for_restore.py", am_conf.g_conf.bh_cluster)
    ...
    def run(self):
        task = bs_restore_task.BsRestoreTask("bs_restore_task.test", self.task_context_, service_id='bs-testenv', options=None)
        ret = task.run()
        print "[FINISH] %d" % ret
