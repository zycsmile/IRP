...
def exec_task(ref_task_inst, context):
    ...
    task = bs_scheduler_task.BsSchedulerTask(context=context, service_id=options['service_id'], 
                                             options=options, ref_task_inst=ref_task_inst, 
                                             scheduler_task_helper=helper, run_mode='rebalance', 
                                             app_id=app_id, app_group_id=app_group_id)
    ret, err = task.run()
