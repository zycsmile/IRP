...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'app':
        if sys.argv[2] == 'describe':
            ...
            for k,v in opts:
                if k == '--app_id':
                    app_id = v
            if app_id == "bs-testenv_se_0" or app_id == "bs-testenv_se_1":
                desc = open("./bs_desc", "r")
                res = desc.read()
                res = res.replace("bs_fake_0", app_id)
                print res
                sys.exit(0)
            else:
                sys.exit(1)
    ...
    if sys.argv[1] == 'resource':
        if sys.argv[2] == 'reconfig':
            f=open("tmp","a")
            f.write(str(sys.argv))
            opts, args = getopt.getopt(sys.argv[3:], 'h', ['app_id=', 'cluster=', 'containers=', 'resource_spec='])
            opts_d = {}
            for k, v in opts:
                opts_d[k] = v
            f.close()

            f = open("resource.%s" % opts_d['--app_id'], "w")
            fi = open(opts_d['--resource_spec'], 'r')
            f.write(fi.read())
            f.close()
            fi.close()
